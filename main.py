...
RESTING_TIME = 60  # Time the fish rests before changing direction
MIN_ANGLE_CHANGE = 10  # Minimum angle change when changing direction
MAX_ANGLE_CHANGE = 45  # Maximum angle change when changing direction
...
class Game:
    ...
    def show_loading_screen(self):
        # Display the loading screen with dependencies status
        loading_text = self.font.render("Loading...", True, (255, 255, 255))
        status_text = self.font.render("Checking dependencies...", True, (255, 255, 255))

        self.screen.fill((0, 0, 0))
        self.screen.blit(loading_text, (SCREEN_WIDTH // 2 - loading_text.get_width() // 2, SCREEN_HEIGHT // 2 - loading_text.get_height() // 2))
...
class FishAnimation:
    def __init__(self, font):
        self.sprite = random.choice(["\\o>", "<o>", "<o/"])
    ...
    def rest(self):
        ...
        if self.resting_time <= 0:
            self.state = "swimming"
            self.angle += random.uniform(-MAX_ANGLE_CHANGE, MAX_ANGLE_CHANGE)
